...
- [x] T001 The colliding verbs are renamed, everywhere
  ...
  - Acceptance:
    - the skill directories become `apply` (was `loop`), `step` (was `run`), `steps` (was `run-auto`) and `audit` (was `review`), while `plan`, `role` and the `ping` / `setup` commands are untouched — `tests/test_skill_names.py` asserts the exact directory set on disk, since that set is what `claude plugin details` reports as the registered identifiers
    - every in-repo reference to the old invocations is updated — `README.md`, `plugins/rein/README.md`, the workflow, the other skills' cross-references and `docs/` — and `tests/test_skill_names.py` greps the whole repository for `/rein:loop`, `/rein:run`, `/rein:run-auto` and `/rein:review` and fails on any survivor outside a changelog or a plan's Why section
...
- [x] T002 A reserved name cannot ship again
  ...
  - Acceptance:
    ...
    - `tests/test_reserved_names.py` holds the list of identifiers Claude Code ships — including `loop`, `run`, `review`, `init`, `simplify`, `schedule` — and it carries a comment saying it is a snapshot, when it was taken, and how to refresh it from `claude plugin details` plus the session's own skill listing, because a list presented as complete would be a claim this repo cannot verify
    - `tests/test_reserved_names.py` fails naming the offender when any skill directory or command file matches that list, and a second test in it asserts the list is non-empty and the sweep actually read the directories, so it cannot pass vacuously
    - the guard covers commands as well as skills: `claude plugin details` counts both as skills, so `commands/loop.md` would collide exactly as a skill directory would — `tests/test_reserved_names.py` sweeps both directories and a test proves it by pointing the sweep at a fixture containing a reserved command file
    - a skill whose name merely CONTAINS a reserved word is allowed — `run-auto` was never the collision, `run` was — and `tests/test_reserved_names.py` pins that distinction so the guard does not start rejecting legitimate names
