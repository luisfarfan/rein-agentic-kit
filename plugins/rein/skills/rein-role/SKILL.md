---
name: rein-role
description: "Assign this session's working role — planner, implementer or reviewer — and load its operating profile. Use when the user runs /rein:rein-role, says which role a session should play, or when you are about to plan, implement or review and it is unclear which one you are."
license: MIT
---
...
enforced by `/rein:rein-step`, `/rein:rein-audit` and the review gate — this skill just makes the
session's role explicit and loads its profile.

Usage: `/rein:rein-role <planner|implementer|reviewer>`
...
1. **Record this invocation** — never blocks, never fails the run. Shell state does NOT
   ...
   ```bash
   R=$(command -v rein || ls -d ~/.claude/plugins/cache/*/rein/*/bin/rein 2>/dev/null | sort -V | tail -1); "$R" event rein-role
   ```
...
Confirm briefly — e.g. `Role set: reviewer. I'll review completed changes via /rein:rein-audit and
