---
name: rein-step
description: "Execute exactly ONE ready task of the plan, verify it, and only then tick its checkbox. One task per invocation, with hard attempt limits. Use when the user asks to implement or run the next task, or invokes /rein:rein-step."
license: MIT
---
...
Usage: `/rein:rein-step [change-name]`
...
1. **Record this invocation** — never blocks, never fails the run. Shell state does NOT
   ...
   ```bash
   R=$(command -v rein || ls -d ~/.claude/plugins/cache/*/rein/*/bin/rein 2>/dev/null | sort -V | tail -1); "$R" event rein-step
   ```
